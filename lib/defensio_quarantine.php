...
<?php 					} else { ?>
							<a href="<?php echo htmlspecialchars( add_query_arg( 'comment_type', 'all' ) ) ?>">Show all</a> 
<?php					}
...
<?php 					} else { ?>
							<a href="<?php echo htmlspecialchars( add_query_arg( 'comment_type', 'comments' ) ) ?>">Just comments</a> 
<?php					}
...
<?php 					} else { ?>
							<a href="<?php echo htmlspecialchars( add_query_arg( 'comment_type', 'trackbacks' ) ) ?>">Just ping/trackbacks</a> 
<?php					} ?>
						<br />
...
<?php 						
							...
							echo " (" . $v['hidden_spam_count'] . "): &nbsp;"; 
?>
							<input type="checkbox" id="defensio_hide_very_spam" name="defensio_hide_very_spam" <?php if ($v['spaminess_filter'] == '1' ) echo " checked=\"1\""; ?> onclick="javascript:this.form.submit();" />
...
<?php			if ($v['type_filter'] == 'comments')
				...
				if($v['spaminess_filter'] and isset($v['hidden_spam_count']) and $v['hidden_spam_count'] > 0)
					echo " However, you are hiding " . $v['hidden_spam_count'] . " obvious spam messages.";
?>
			</p>
<?php 
		} else {
...
<?php
					...
					foreach($v['comments'] as $spam_comment) {
						...
						if( $v['order'] == 'post_date' and (  $last_comment_post == null or ( $last_comment_post != $spam_comment->post_id ))) {
							...
							echo "<li class=\"defensio_post_title\"> $spam_comment->post_title <span class=\"defensio_post_date\"> ($spam_comment->post_date) </span></li>";
						}
...
function defensio_render_page_navigation_links($v) {
	...
	if ($v['current_page'] > 1) {
		...
		$output .= "<a class=\"prev\" href=\" " . htmlspecialchars( add_query_arg(array('sort_by' => $sort_by, 'defensio_page' => $previous_page, 'defensio_search_query' => $v['query'])) ) . "\">&laquo; Previous Page</a>&nbsp;&nbsp;";
	}
	...
	if ($v['pages_count'] > 2) {
		...
		while ($i <= $v['pages_count']) {
			if(defensio_show_page_number($i, $v['pages_count'], $v['current_page'])) {
				if ($i != $v['current_page']) {
					$output .= " <a class=\"page-numbers\" href=\"" . htmlspecialchars( add_query_arg ( array('sort_by' => $sort_by, 'defensio_page' => $i, 'defensio_search_query' => $v['query'] )) ) . "\">  $i </a>  ";
				} else {
	...
	if($v['current_page'] < $v['pages_count'] ) {
		...
		$output .= "<a class=\"next\" href=\"" . htmlspecialchars(add_query_arg (array('sort_by' => $sort_by, 'defensio_page' => $next, 'defensio_search_query' => $v['query']))) . "\" >&nbsp;&nbsp;Next Page &raquo;</a> ";
	}
